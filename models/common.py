...
class BiFPN_Add3(nn.Module):
    ...
    def forward(self, x):
        weights = torch.softmax(self.w, dim=0) * 2
        # weights = w / (torch.sum(w, dim=0) + self.epsilon)
        out = torch.cat((x[0], x[1]), dim=1)
        # Fast normalized fusion
        return self.conv(self.silu(weights[0] * out + weights[1] * x[2]))
...
# =======================INSN模块=============================#
class ELAN_M(nn.Module):
    def __init__(self, c1, c2, shortcut=True, e=0.25):
        super().__init__()
        c_ = int(c1 * e)
        self.cv1 = Conv(c1, c_, 1, 1)
...
class INSNs(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=False, e=0.25):
        ...
        self.conv2_1 = Conv(c_, c_, 1, 1)
        self.conv2_2 = DWConv(c_, c_, 3, 1)
        self.conv3 = Conv(c1, c2, 1, 1)
        self.elan = nn.Sequential(*(ELAN_M(c1, c_, shortcut, e=e) for _ in range(n)))
    def forward(self, x):
        x1,x2 = self.elan(x)
        x3 = self.conv1(x)
        x3_1 = self.conv2_1(x3)
        x3_2 = self.conv2_2(x3)
        out = self.conv3(torch.cat((x1,x2,x3_1,x3_2), dim=1))
        return out


class ELAN_A(nn.Module):
    # Standard bottleneck
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
...
class INSNl(nn.Module):
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        ...
        self.conv8 = Conv(c1, c2, 1, 1)
        self.elan = nn.Sequential(*(ELAN_A(c1, c1*e, shortcut, g, e=e) for _ in range(n)))
    def forward(self, x):  # 1*64*160*160
        x1,x2,x3 = self.elan(x)
        x4 = self.conv1(x)
        x4_1 = self.conv2(x4)
        x4_2 = self.conv4(self.conv3(x4))
        x4_3 = self.conv7(self.conv6(self.conv5(x4)))
        out = self.conv8(torch.cat((x1, x2, x3, x4 + x4_1 + x4_2 + x4_3), dim=1))
        return out
# ======================= 解耦头=============================#
